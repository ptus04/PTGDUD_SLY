...
import FeaturedCarouselIndicator from "./CarouselIndicator";
...
const FeaturedCarousel = () => {
  ...
  const handleOrientationChanged = useCallback(async () => {
    ...
    const res = await fetch(`/api/products/carousel?orientation=${orientation}`);
  ...
  useEffect(() => {
    ...
    return () => screen.orientation.removeEventListener("change", handleOrientationChanged);
  }, [handleOrientationChanged]);

  useEffect(() => {
    const interval = setInterval(() => setActive((active) => (active + 1) % items.length), 5000);
  ...
  return (
    <div className="relative h-dvh">
      <div className="h-dvh">
        {items?.map((item, i) => <FeaturedCarouselItem key={item._id} item={item} isActive={active === i} />)}
      </div>
