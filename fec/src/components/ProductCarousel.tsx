...
const ProductCarousel = ({ product }: ProductCarouselProps) => {
  ...
  const handlePrevImage = () => setPreviewIndex((prev) => (prev - 1 + product.images.length) % product.images.length);
  const handleNextImage = () => setPreviewIndex((prev) => (prev + 1) % product.images.length);
